...
%                     [H_I, H_R, h_dir, H_los, H_R1] = Rician_channel(di, dr, N, N_f, L, path_exp, ref_loss_db, ric_fact, f0);
                    ...
                    for out_iter = 1:100
                        ...
                        for scaiter = 1:100
                            cvx_begin sdp quiet
                                variable Xopt(elem_num, elem_num) hermitian semidefinite
                                minimize trace(K1*Xopt)
                                subject to 
                                    for i = 1:N
                                        for j = i:N
                                            Pi = P((i-1)*N + (1:N), :);
                                    ...
                                    for m = 1:elem_num
                                        for n = 1:elem_num
                                            approxX = approxX + 2*real((0.5*X(m,n)/norm(X, 'fro'))*(Xopt(m,n) - X(m,n))');
                                        end
                                    end
                                    norm_nuc(Xopt) - approxX <= rank_thr
                            cvx_end
                            X = Xopt;
                            test_X = [];
                            for i = 1:N
                            ...
                            if norm(1 - trace(K1*X)/fin) <= phase_thr
                                break
                            end
                            fin = trace(K1*X);
                            K1 = myJ + myJ';
                    ...
                    disp(strcat('SDP_Rank_Final_RIS_', string(N), '_MODE_Power', string(MODE_P), '_MODERIS_', string(MODE_CON), ...
                        '_sub_carrier_', string(N_f), '_realization_', string(realiz)))
                    filename = strcat('SDP_Rank_Final_RIS_', string(N), '_MODE_Power', string(MODE_P), '_MODERIS_', string(MODE_CON), ...
