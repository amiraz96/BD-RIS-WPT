...
%                     [H_I, H_R, h_dir] = Rayleigh_channel_time(di, dr, N,N_f, L, path_exp, ref_loss_db, f0, BW, alph_chan);
                    ...
                    [Q, Lambda] = eig(PS);
                    PS = takagi_phase_randomization_approximation(PS, 200000, H_R, H_I, w0, K2, K4);
                    equiv_chan = zeros(N_f, 1);
                    for n = 1:N_f
                        equiv_chan(n) = H_R(:, n).'*PS*H_I(:, n) + h_dir(n);
                    end
                    ...
                    disp(strcat('SDR_Single_Final_RIS_', string(N), '_MODE_Power', string(MODE_P), '_MODERIS_', string(MODE_CON), ...
                        '_sub_carrier_', string(N_f), '_realization_', string(realiz)))
                    filename = strcat('SDR_Single_Final_RIS_', string(N), '_MODE_Power', string(MODE_P), '_MODERIS_', string(MODE_CON), ...
